...
uint8_t read_register(uint8_t reg)
{
	...
	HAL_SPI_TransmitReceive(&hspi3, (uint8_t*)0xff, &dt, 1, 1000);
	csn(HIGH);
	return dt;
}
...
uint8_t write_registerMy(uint8_t reg, const uint8_t* buf, uint8_t len)
{
	...
	csn(LOW);
	HAL_SPI_TransmitReceive(&hspi3, &addr, &status, 1, 100);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)buf, len, 100);
	csn(HIGH);
	return status;
}
...
uint8_t write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
	...
	csn(LOW);
	HAL_SPI_TransmitReceive(&hspi3, &addr, &status, 1, 100);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Transmit(&hspi3, &empt, 1, 100);
	}
...
uint8_t read_payload(void* buf, uint8_t data_len)
{
	...
	HAL_SPI_Receive(&hspi3, (uint8_t*)current, data_len, 100);

	while(blank_len--)
	{
		uint8_t empt = 0;
		HAL_SPI_Receive(&hspi3, &empt, 1, 100);
	}
...
uint8_t spiTrans(uint8_t cmd)
{
	uint8_t status = 0;
	csn(LOW);
	HAL_SPI_TransmitReceive(&hspi3, &cmd, &status, 1, 1000);
	csn(HIGH);
	return status;
}
...
uint8_t NRF_Init(void)
{
	...
	ce(LOW);
	csn(HIGH);
	HAL_Delay(50); // !!!!! TODO: изменен SysTick
...
bool write(const void* buf, uint8_t len)
{
	...
	while(!(get_status() & ((1 << TX_DS) | (1 << MAX_RT))) && isPowerUp())
	{}
...
uint8_t getDynamicPayloadSize(void)
{
	...
	HAL_SPI_TransmitReceive(&hspi3, (uint8_t*)0xff, &result, 1, 1000);
	csn(HIGH);

	if(result > 32)
	{
		flush_rx();
		HAL_Delay(20); // !!!!! TODO: изменен SysTick
		return 0;
	}
...
void writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
	...
	csn(LOW);
	HAL_SPI_Transmit(&hspi3, &addr, 1, 1000);
	HAL_SPI_Transmit(&hspi3, (uint8_t*)current, data_len, 1000);
	csn(HIGH);
}
