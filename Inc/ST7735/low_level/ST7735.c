...
void ST7735_SendDataMultiple(uint8_t *data, uint32_t num)
{
  ...
  //while (HAL_SPI_GetState(&ST7735_SPI_PORT) == HAL_SPI_STATE_BUSY_TX);

  //HAL_SPI_Transmit_DMA(&ST7735_SPI_PORT, data, num);
...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data) {
    ...
    ST7735_Select();
    ST7735_SetAddressWindow(x, y, x+w-1, y+h-1);
    ST7735_SendDataMultiple((uint8_t*)data, sizeof(uint16_t)*w*h);
